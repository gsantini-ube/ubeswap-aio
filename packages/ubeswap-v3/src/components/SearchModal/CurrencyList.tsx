...
function Balance({ balance }: { balance: CurrencyAmount<Currency> }) {
    return (
        <span className="balance-value" title={balance.toExact()}>
            {balance.toSignificant(4)}
        </span>
    );
}

function CurrencyRow({ currency, onSelect, isSelected, otherSelected, style, showCurrencyAmount }: CurrencyRowProps) {
    ...
    return (
        <div className={`currency-row flex-s-between p-1 br-8 mv-05 token-item-${key}`} onClick={() => (isSelected ? null : onSelect())} data-disabled={isSelected} data-selected={otherSelected}>
            <div className={"f f-ac"}>
                <CurrencyLogo currency={currency as WrappedCurrency} size={"24px"} fromList />
                <div className={"f c ml-05"}>
                    <Text title={currency.name} fontWeight={500}>
                        {currency.symbol}
                    </Text>
                    <span className={"fs-075"}>{!currency.isNative && !isOnSelectedList && customAdded ? `${currency.name} • Added by user` : "CELO"}</span>
                </div>
            </div>
            <TokenTags currency={currency} />
            {showCurrencyAmount && <RowFixed style={{ justifySelf: "flex-end" }}>{balance ? <Balance balance={balance} /> : account ? <Loader /> : null}</RowFixed>}
        </div>
    );
}
...
export default function CurrencyList({
    ...
    return (
        <FixedSizeList height={height} ref={fixedListRef as any} width="100%" itemData={itemData} itemCount={itemData.length} itemSize={56} itemKey={itemKey} overscanCount={5}>
            {Row}
        </FixedSizeList>
    );
}
