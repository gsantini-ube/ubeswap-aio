...
export default function LiquidityChartRangeInput({
    ...
    return (
        <AutoColumn gap="md" style={{ minHeight: "260px" }}>
            {isUninitialized ? (
                <InfoBox message={"Your position will appear here."} icon={<Inbox size={56} />} />
            ) : isLoading ? (
                <InfoBox icon={<Loader size="40px" stroke={"white"} />} />
            ) : isError ? (
                <InfoBox message={`Liquidity data not available.`} icon={<CloudOff size={56} />} />
            ) : !formattedData || formattedData === [] || !price ? (
                // <InfoBox message={`There is no liquidity data.`} icon={<BarChart2 size={56} />} />
                <ChartWrapper>
                    <Chart
